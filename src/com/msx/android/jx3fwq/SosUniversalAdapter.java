...
public abstract class SosUniversalAdapter extends BaseAdapter implements
		SectionIndexer, OnScrollListener {
	public static final String TAG = SosUniversalAdapter.class.getSimpleName();

	public interface HasMorePagesListener {
		/**
		 * 不需要加载更多的数据库时， 
		 * {@link SosUniversalAdapter}自动调用此方法
		 */
		void noMorePages();
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (view instanceof SosUniversalListView) {
			((SosUniversalListView) view).configureHeaderView(firstVisibleItem);
		}
	}
	 ...
	 * call {@link SosUniversalListView#tellNoMoreData()} if there is no more
	 * pages.
	 * 
	 * @param page
